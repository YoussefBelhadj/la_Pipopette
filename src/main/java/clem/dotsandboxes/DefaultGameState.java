...
import static clem.util.Comparables.max;
import clem.dotsandboxes.textinterface.Values.BoardRepresentation;
...
import static clem.dotsandboxes.DotsAndBoxesUtils.gridEdgeCount;
...
public class DefaultGameState implements GameState {
	 ...
	 */
	private DefaultGameState(int width, int height, 
			List<Player> players) {
	 ...
	 * @param player
	 */
	private DefaultGameState(DefaultGameState base, Edge edge, Player player) {
		mWidth = base.mWidth;
	 ...
	 */
	public static DefaultGameState get(int width, int height, 
			List<Player> players) {
		return new DefaultGameState(width, height, players);
	}
	...
	@Override
	public DefaultGameState withEdge(Edge edge, Player player) {
		...
		// for the job.
		return new DefaultGameState(this, edge, player);
	}
	 ...
	 * 
	 * <p>Horizontal edges are mapped onto the first clemf of the array, vertical 
	 * edges onto the second clemf.
	 * 
	...
	private Edge edgeForIndex(int index) {
		...
		return DefaultEdge.obtain(x, y, direction);
	}
	...
	private EdgeData getEdgeData(int x, int y, Direction direction) {
		...
		if(index < 0 || index >= mEdges.length)
			throw new IndexOutOfBoundsException(String.format("The specified " +
					...
					DefaultEdge.obtain(x, y, direction)));
