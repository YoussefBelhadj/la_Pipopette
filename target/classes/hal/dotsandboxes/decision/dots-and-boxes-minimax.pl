...
% Import some libraries the code uses. I use AVL tree library assoc to maintain 
% the board state.
...
% Use the lists library to get some common list operations (e.g. length).
:- use_module(library(lists)).
% We need the to generate random numbers to select randomly from optimal moves
...
% Gives runtime statistics on the time taken to run +Goal.
time_(Goal):-
